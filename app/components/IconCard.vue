<template>
  <div class="icon-card" :class="{ dark }">
    ...
    <a href="#" @click.prevent="toggleDark" class="action dark">
      <icon :icon="dark ? 'lightbulb_on' : 'lightbulb_off'"></icon>
    </a>
    <a href="#" @click.prevent="closeCard" class="action close">
      <icon>delete</icon>
    </a>
    <h2><small>Icon name:</small>{{ camelCase(icon.liga) }}</h2>
    <table>
      <tr class="icons">
        <td>
          <icon class="zoom-1x">{{ icon.liga }}</icon>
        </td>
        <td>
          <icon class="zoom-2x">{{ icon.liga }}</icon>
        </td>
        <td>
          <icon class="zoom-4x">{{ icon.liga }}</icon>
        </td>
        <td>
          <icon class="zoom-8x">{{ icon.liga }}</icon>
        </td>
      </tr>
      <tr class="sizes">
        <td>
    ...
    <icon-svg :size="256" :dark="dark" :icon="icon"></icon-svg>
    <p>
      <small>HTML Markup for ligature</small>
      <code>&lt;i class=&quot;icon&quot;&gt;{{ icon.liga }}&lt;/i&gt;</code>
    </p>
    <p>
